...
class ImageMasker():
    ...
    def mask_all_regions(self, source, regions_to_mask, mask_method, blur_foreground_background):
        output = source.copy()
        ...
        if blur_foreground_background == 'foreground':
            ...
            if not combine_via_mask:
                for masking_region in regions_to_mask:
                    ...
                    output[start[1]:end[1],start[0]:end[0]] = source[start[1]:end[1],start[0]:end[0]]
            else:
                for masking_region in regions_to_mask:
                    cv2.rectangle(mask, masking_region['start'], masking_region['end'], (255), -1)
